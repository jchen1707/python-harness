...
2. **Open a progress file** at `.agents/plans/loop-<goal>.md` (gitignored). It
...
| Goal | Stop condition |
| --- | --- |
| `docs` | Every claim in `AGENTS.md`, `README.md` and `docs/architecture.md` matches the code; every documented command runs; no reference to a deleted file or command |
...
the `Stop` hook (`.agents/hooks/verify.py`) to keep the loop honest — it blocks

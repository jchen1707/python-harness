...
Run it from the repo whose sessions you want to recover:
    ...
    uv run python .agents/hooks/distil_backlog.py          # dry run: list only
    uv run python .agents/hooks/distil_backlog.py --run    # distil (spends tokens)
...
fixing a writer does not remove what it already wrote.
    ...
    uv run python .agents/hooks/distil_backlog.py --audit         # report only
    uv run python .agents/hooks/distil_backlog.py --audit --run   # delete echo notes
...
# Sibling module: hooks run as `python <abs path>/distil_backlog.py`, so `.agents/hooks`

...
Rules that apply to one layer live in that layer's `CLAUDE.md`, next to the code they govern.
...
This choice deviates from the hybrid search rule in `src/app/ai/retrieval/CLAUDE.md`.
...
This choice deviates from the metrics rule in `src/app/core/CLAUDE.md`.

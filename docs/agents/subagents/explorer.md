...
├── ai/             ← retrieval, reranking, agents, evals (each with its own AGENTS.md)
...
Every layer directory carries its own `AGENTS.md`. Those are path-scoped: reading one tells
...
`pyright` is available as an MCP server, and `AGENTS.md` has a section on preferring it over

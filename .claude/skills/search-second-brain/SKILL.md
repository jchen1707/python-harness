...
1. **Read `_INDEX.md` first. Always.** It is one row per note — date, project, one-line
   summary, link — and it is the cheapest thing in the directory. It tells you which
   notes are worth opening before you spend a token on any of them.

   The index is generated by the hook and rebuilt on every write, so it is current. If it
   is missing, the hook has not run yet; fall back to step 2.

   Never read every note to answer one question. That cost is exactly what the index
   exists to avoid, and it grows with every session.

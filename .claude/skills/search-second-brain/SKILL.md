...
root and its learnings directory:

- Learnings written by the `SessionEnd` hook: `$CLAUDE_LEARNINGS_DIR`
  (in PowerShell, `$env:CLAUDE_LEARNINGS_DIR`). Each note has
  `tags: [project-learnings, session-retro]` and two sections —
  *Implementation learnings* and *Architecture & design learnings*.
- The wider vault around it — sibling directories hold hand-written notes that are often
  more considered than anything auto-distilled.

If `CLAUDE_LEARNINGS_DIR` is unset, say so and stop; there is nothing to search. Do not
guess at a path.
...
1. **Search wide before reading deep.** `Grep` the whole vault for the topic and for its
...
2. **Read the hits in full.** These notes are short. Skimming a lesson is how you get the
   correction backwards.
3. **Prefer the specific over the recent.** A note that names the exact tool or failure
   beats a newer one that gestures at the area.
4. **Check the repo too.** `CLAUDE.md`, `docs/architecture.md` and `docs/agents/` may

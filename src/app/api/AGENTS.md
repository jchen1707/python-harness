...
- Group routes by resource in `api/routes/<resource>.py`. Register each router in the app
  factory.
- Bind `request_id` to the logger at the start of the request. See `core/AGENTS.md`.
...
Substitute the service with a fake through the dependency override. See `tests/AGENTS.md`.
